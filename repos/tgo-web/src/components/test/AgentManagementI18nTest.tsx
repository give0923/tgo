      ...
      <div className="space-y-6">
        ...
        <section className="border p-4 rounded">
          ...
          <div className="space-y-2">
            <p><strong>Title:</strong> {t('agents.title', '智能体管理')}</p>
            <p><strong>Create:</strong> {t('agents.actions.create', '创建智能体')}</p>
            <p><strong>Refresh:</strong> {t('agents.actions.refresh', '刷新')}</p>
            <p><strong>Edit:</strong> {t('agents.actions.edit', '编辑智能体')}</p>
            <p><strong>Delete:</strong> {t('agents.actions.delete', '删除智能体')}</p>
          </div>
        </section>
        ...
        <section className="border p-4 rounded">
          ...
          <div className="space-y-2">
            ...
            <p><strong>Load Failed Desc:</strong> {t('agents.messages.loadFailedDesc', '无法加载智能体列表，请稍后重试')}</p>
            <p><strong>Refresh Success:</strong> {t('agents.messages.refreshSuccess', '刷新成功')}</p>
            <p><strong>Create Success:</strong> {t('agents.messages.createSuccessDesc', '智能体 "{name}" 已成功创建', { name: 'Test Agent' })}</p>
            <p><strong>Delete Success:</strong> {t('agents.messages.deleteSuccessDesc', '智能体 "{name}" 已删除', { name: 'Test Agent' })}</p>
          </div>
        </section>
        ...
        <section className="border p-4 rounded">
          ...
          <div className="space-y-2">
            <p><strong>Default Role:</strong> {t('agents.card.defaultRole', '智能体')}</p>
            ...
            <p><strong>Edit Tooltip:</strong> {t('agents.card.editTooltip', '编辑智能体')}</p>
        ...
        <section className="border p-4 rounded">
          ...
          <div className="space-y-2">
            <p><strong>Title:</strong> {t('agents.empty.title', '暂无智能体')}</p>
            <p><strong>Description:</strong> {t('agents.empty.description', '点击「创建智能体」按钮开始创建您的第一个智能体')}</p>
          </div>
        </section>
        ...
        <section className="border p-4 rounded">
          ...
          <div className="space-y-2">
            <p><strong>Create Title:</strong> {t('agents.modal.create.title', '创建智能体')}</p>
            <p><strong>Edit Title:</strong> {t('agents.modal.edit.title', '编辑智能体')}</p>
            <p><strong>Delete Title:</strong> {t('agents.modal.delete.title', '删除智能体')}</p>
            <p><strong>Delete Message:</strong> {t('agents.modal.delete.message', '确定要删除智能体 "{name}" 吗？此操作不可撤销。', { name: 'Test Agent' })}</p>
