    ...
    <div className={`flex items-center justify-between p-3 rounded-md border transition-colors ${
      ...
      <div className="flex items-start space-x-3 min-w-0 flex-1">
        ...
        <div className="min-w-0">
          <div className={`text-sm font-medium ${enabled ? 'text-gray-800' : 'text-gray-500'} truncate`}>{kb.title}</div>
          <div className="text-xs text-gray-500 mt-1 leading-snug line-clamp-2">{kb.content}</div>
          <div className="text-xs text-gray-400 mt-1 truncate">{kb.views} 文件 • {kb.author}</div>
        </div>
      </div>
