        ...
        <div className="flex items-center justify-between p-6 border-b border-gray-200">
          <div className="flex items-center space-x-4">
            ...
            <div>
              ...
              <p className="text-sm text-gray-600">{agent.role || t('agents.card.defaultRole', '智能体')}</p>
            </div>
        ...
        <div className="p-6 space-y-6">
          ...
          <div>
            ...
            <div className="bg-gray-50 rounded-lg p-4 space-y-3">
              <div>
                <label className="text-sm font-medium text-gray-700">{t('agents.modal.detail.fields.description', '描述')}</label>
                <p className="text-sm text-gray-600 mt-1">{agent.description}</p>
              </div>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  <label className="text-sm font-medium text-gray-700">{t('agents.modal.detail.fields.agentId', '智能体ID')}</label>
                  <p className="text-sm text-gray-600 font-mono mt-1">{agent.id}</p>
                </div>
                <div>
                  <label className="text-sm font-medium text-gray-700">{t('agents.modal.detail.fields.llmModel', 'LLM模型')}</label>
                  <p className="text-sm text-gray-600 font-mono mt-1">{agent.llmModel || 'gemini-1.5-pro'}</p>
                </div>
                <div>
                  <label className="text-sm font-medium text-gray-700">{t('agents.modal.detail.fields.endpoint', '端点地址')}</label>
                  <p className="text-sm text-gray-600 font-mono mt-1">{agent.endpoint || 'N/A'}</p>
                </div>
                <div>
                  <label className="text-sm font-medium text-gray-700">{t('agents.modal.detail.fields.lastActive', '最后活跃')}</label>
                  <p className="text-sm text-gray-600 mt-1">{agent.lastActive || 'N/A'}</p>
                </div>
              </div>
            </div>
          </div>
          ...
          <div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <div className="bg-blue-50 rounded-lg p-4 text-center">
                ...
                <div className="text-2xl font-bold text-blue-900">{agent.conversationCount || 0}</div>
                <div className="text-sm text-blue-600">{t('agents.modal.detail.metrics.conversationCount', '对话次数')}</div>
              </div>
              <div className="bg-green-50 rounded-lg p-4 text-center">
                <Activity className="w-8 h-8 text-green-600 mx-auto mb-2" />
                <div className="text-2xl font-bold text-green-900">{agent.successRate || 0}%</div>
                <div className="text-sm text-green-600">{t('agents.modal.detail.metrics.successRate', '成功率')}</div>
              </div>
              <div className="bg-purple-50 rounded-lg p-4 text-center">
                <Clock className="w-8 h-8 text-purple-600 mx-auto mb-2" />
                <div className="text-2xl font-bold text-purple-900">{agent.responseTime || '0s'}</div>
                <div className="text-sm text-purple-600">{t('agents.modal.detail.metrics.avgResponseTime', '平均响应时间')}</div>
              </div>
            </div>
          </div>
              ...
              {agent.knowledgeBases && agent.knowledgeBases.length > 0 ? (
                <div className="flex flex-wrap gap-2">
                  {agent.knowledgeBases.map((kbId) => (
                    <span
                      key={kbId}
                      className="inline-flex items-center px-3 py-1 rounded-md bg-blue-100 text-blue-800 text-sm"
                    >
          ...
          {agent.tags && agent.tags.length > 0 && (
            <div>
              <h3 className="text-lg font-medium text-gray-900 mb-3">{t('agents.modal.detail.tags', '标签')}</h3>
              <div className="flex flex-wrap gap-2">
